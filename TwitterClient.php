...
class TwitterClient {
    ...
    private $userId;
    private $screenName;
    private $cookieExpiry = 86400; // 1 day
    ...
    private function checkRememberMeCookie() {
        if (!$this->isLoggedIn() && isset($_COOKIE['flint_remember'])) {
            $cookieData = json_decode($_COOKIE['flint_remember'], true);
    ...
    public function logout() {
        ...
        if (isset($_COOKIE['flint_remember'])) {
            setcookie('flint_remember', '', time() - 3600, '/');
        }
    }
    ...
    public function getUserProfile($screenName = null) {
        ...
        if ($screenName) {
            ...
            if (!$this->isLoggedIn()) {
                ...
                curl_setopt($curl, CURLOPT_USERAGENT, 'DUTB-Web/1.0');
    ...
    public function getUserTweets($screenName = null, $count = 20, $max_id = null) {
        ...
        if ($screenName && !$this->isLoggedIn()) {
            ...
            curl_setopt($curl, CURLOPT_URL, $fullUrl);
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl, CURLOPT_USERAGENT, 'Flint-Web/1.0');
            curl_setopt($curl, CURLOPT_HEADER, false);
            curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, true);
            curl_setopt($curl, CURLOPT_TIMEOUT, 30);
    ...
    public function getTrendingTopics() {
        $url = $this->apiUrl . '/1/trends/place.json';
        $params = array('id' => 1);
    ...
    private function makeRequest($method, $url, $params = array(), $auth = true, $multipart = false) {
        ...
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_USERAGENT, 'Flint-Web-Mobile/1.0');
        curl_setopt($curl, CURLOPT_HEADER, false);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, true);
        curl_setopt($curl, CURLOPT_TIMEOUT, 30);
